...
- [Проблема в самих тестах!](#%D0%BF%D1%80%D0%BE%D0%B1%D0%BB%D0%B5%D0%BC%D0%B0-%D0%B2-%D1%81%D0%B0%D0%BC%D0%B8%D1%85-%D1%82%D0%B5%D1%81%D1%82%D0%B0%D1%85)
  - [Немного истории](#%D0%BD%D0%B5%D0%BC%D0%BD%D0%BE%D0%B3%D0%BE-%D0%B8%D1%81%D1%82%D0%BE%D1%80%D0%B8%D0%B8)
  - [Небольшая викторина](#%D0%BD%D0%B5%D0%B1%D0%BE%D0%BB%D1%8C%D1%88%D0%B0%D1%8F-%D0%B2%D0%B8%D0%BA%D1%82%D0%BE%D1%80%D0%B8%D0%BD%D0%B0)
- [Обзор TDD](#%D0%BE%D0%B1%D0%B7%D0%BE%D1%80-tdd)
  - [Тесты это формализованные требования](#%D1%82%D0%B5%D1%81%D1%82%D1%8B-%D1%8D%D1%82%D0%BE-%D1%84%D0%BE%D1%80%D0%BC%D0%B0%D0%BB%D0%B8%D0%B7%D0%BE%D0%B2%D0%B0%D0%BD%D0%BD%D1%8B%D0%B5-%D1%82%D1%80%D0%B5%D0%B1%D0%BE%D0%B2%D0%B0%D0%BD%D0%B8%D1%8F)
  ...
  - [Тесты это лучшая документация](#%D1%82%D0%B5%D1%81%D1%82%D1%8B-%D1%8D%D1%82%D0%BE-%D0%BB%D1%83%D1%87%D1%88%D0%B0%D1%8F-%D0%B4%D0%BE%D0%BA%D1%83%D0%BC%D0%B5%D0%BD%D1%82%D0%B0%D1%86%D0%B8%D1%8F)
  - [Промежуточный вывод](#%D0%BF%D1%80%D0%BE%D0%BC%D0%B5%D0%B6%D1%83%D1%82%D0%BE%D1%87%D0%BD%D1%8B%D0%B9-%D0%B2%D1%8B%D0%B2%D0%BE%D0%B4)
- [В чём же решение?](#%D0%B2-%D1%87%D1%91%D0%BC-%D0%B6%D0%B5-%D1%80%D0%B5%D1%88%D0%B5%D0%BD%D0%B8%D0%B5)
    - [Парочка примеров](#%D0%BF%D0%B0%D1%80%D0%BE%D1%87%D0%BA%D0%B0-%D0%BF%D1%80%D0%B8%D0%BC%D0%B5%D1%80%D0%BE%D0%B2)
...
## Проблема в самих тестах!
Исходя из [этого исследования](https://stateofjs.com/) найнизшая общая удовлетворённость во всей экосистеме принадлежит именно инструментам для тестирования. Так было в 2016 и 2017 годах. Я не нашёл более ранних иследований, но это уже не очень важно.
...
Для сравнения.  
В 2010 зарелизился angular.  
Ember появился в 2011.  
react - 2013.  
И так далее…

> Во время написанния этой статьи не был создан ни один JS фреймворк...  
...
[TDD](https://en.wikipedia.org/wiki/Test-driven_development) не намного моложе, [если не старше](https://www.quora.com/Why-does-Kent-Beck-refer-to-the-rediscovery-of-test-driven-development-Whats-the-history-of-test-driven-development-before-Kent-Becks-rediscovery). Всё это приводит нас к тому, что мы уже можем объективно оценить все плюсы и минусы.
...
Так что **"Тесты это формализованные требования"** - правда только тогда, когда эти тебования существуют _до_ начала самой разработки, как в "модели Водопад" или проектах [NASA](https://www.nasa.gov/), где "клиенты" это ученые и инженеры.

> В определённых условиях это будет работать и с "Agile" процессами. Особенно, если что-нибудь по типу [BDD](https://en.wikipedia.org/wiki/Behavior-driven_development) будет использованно, но это уже совсем другая история.
...
С другой стороны, слабые разработчики будут создавать низкокачественный код, покрытый низкокачественными тестами, потому что создание хороших тестов это своего рода искусство, как и само програмированние.
...
> Обратите внимание, мы использовали 15 строк кода, что бы ничего не протестировать.

Но и этот тест не сделат дизайн вашей системы лучше:
```JavaScript
...
```
Найбольшая проблема этого теста это изначальная кодовая база, но даже в таком случае его можно было существенно улучшить, даже без рефакторинга уже работающего проекта.
...
Давайте предположим, что все в проекте достаточно хорошо влдеют выбранным тестовым инструментом, методологией TDD и лучшими практиками юнит-тестирования.  
...
А менджмент готов решать все организационные проблемы, вызванные TDD (например более длинный процесс ввода новых разработчиков в команду).
...
Но в реальном мире, который очень динамичен, требования могут измениться и то, что было корректным поведением раньше, станет некорректным. В этом случае вам необходимо переписать требования в связи с новыми реалиями. И, очевидно, потратить новые усилия, которые не окупятся сразу же.

Вы даже можете попасть в цикл подобного типа:

![tdd-cycle-for-changes-wrong](./images/tdd-cycle-for-changes-wrong.svg)

Ладно, этот цикл не следует принципам TDD. Но следующий уже не противоречит:
...
Как вы думаете, что в них общего? - Они обе построенны на **примерах кода**. И даже более того. Все это примеры можно легко запустить (angular использует [StackBlitz](https://stackblitz.com), а react - [CodePen](https://codepen.io)), так что вы можете увидеть, что оно даёт на выходе и что произойдёт, если вы что-то измените.  
Конечно же, там так же есть и простой текст, но это как комментарии в коде - они вам нужны только если вы что-то не поняли из кода.

_**Исполняемые примеры кода**_ - вот лучшая документация!

Тесты близки к этому, но не досточно.
```TypeScript
...
Давайте будем честными, пример теста выше намного менее читабельный чем настоящая документация. И проблема не в этом конкретном тесте - я уверен, что ребят из [facebook](https://www.facebook.com/) знают как писать хороший код и хорошие тесты :) Весь этот мусор из инструментов тестировани и assertion библиотек, как `it`, `describe`, `test`, `to.be.true` просто захламляет ваши тесты.

> Кстати, есть библиотека, которая называется [tape](https://github.com/substack/tape) с минималистичным API, потому что лубой тест можно переписать используя только `equal`/`deepEqual`, а думать в этих терминах это в целом хорошая практика для юнит-тестирования. Но дажем тестам для `tape` ещё очень далеко до просто **исполняемых примеров кода**.
...
2. **TDD поощряет хорошую архитектуру** если разработчики достаточно квалифицированны;
...
Базовая концепция очень простая. Пишите код:
```TypeScript
export function sampleFn(a: any, b: any) {
    return a + b + b + a;
}
```
И просто используйте его в вашеме тесте:
```TypeScript
...
Все последующие прогонки тестов буду сравнивать существующий **baseline** со значениями єкспортированными из ваших тестов. Если они отличаются, тест провален, иначе - пройден.  
Если требования изменились, просто измените кода, прогоните тесты и примите **новый baseline**.
...
Только около 40% от запланированного было реализовано. Но вся базовая функциональность уже работает, так можете попробовать поигратся с ней. Может быть вам даже понравится, кто знает?
