...
5. А менджмент готов решать все организационные проблемы, вызванные TDD (например более длинный процесс ввода новых разработчиков в команду).
...
Как вы думаете, что в них общего? - Они обе построенны на **примерах кода**. И даже более того. Все эти примеры можно легко запустить (angular использует [StackBlitz](https://stackblitz.com), а react - [CodePen](https://codepen.io)), так что вы можете увидеть, что оно даёт на выходе и что произойдёт, если вы что-то измените.  
...
Тесты близки к этому, но не достаточно.
```TypeScript
...
> Кстати, есть библиотека, которая называется [tape](https://github.com/substack/tape) с минималистичным API, потому что любой тест можно переписать используя только `equal`/`deepEqual`, а думать в этих терминах это в целом хорошая практика для юнит-тестирования. Но дажем тестам для `tape` ещё очень далеко до просто **исполняемых примеров кода**.
...
Только около 40% от запланированного было реализовано. Но вся базовая функциональность уже работает, так что можете попробовать поигратся с ней. Может быть вам даже понравится, кто знает?
